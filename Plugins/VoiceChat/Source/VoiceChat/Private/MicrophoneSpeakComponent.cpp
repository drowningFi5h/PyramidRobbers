...
bool UMicrophoneSpeakComponent::initAudioResources(int32 _voiceSampleRate, int32 _voiceNumChannels, int32 opusFramesPerSec) {
	UE_LOG(LogVoiceChatPro, Warning, TEXT("[VoiceChatPlugin] UMicrophoneSpeakComponent::initAudioResources CALLED. SampleRate: %d, Channels: %d, OpusFPS: %d"), _voiceSampleRate, _voiceNumChannels, opusFramesPerSec);
	...
	if (wasInitAudioResources) {
		...
		UE_LOG(LogVoiceChatPro, Display, TEXT("UMicrophoneSpeakComponent::initAudioResources recreate desktop capture"));
...
#	if (ENGINE_MAJOR_VERSION == 4) && (ENGINE_MINOR_VERSION >=26)
	...
	VoiceSoundWaveProcedural->SoundClassObject = APlayerVoiceChatActor::voiceChatSoundClass;
	...
	SetVoiceSoundClass(APlayerVoiceChatActor::voiceChatSoundClass);
	...
	if (GetNetMode() != NM_DedicatedServer && UUniversalVoiceChat::GetMyPlayerVoiceActor() != NULL && UUniversalVoiceChat::GetMyPlayerVoiceActor() == (AActor*)GetOwner()){
		...
		if (PCVoiceCapture.IsValid() && PCVoiceCapture->IsCapturing()) {
			...
			if (CaptureState != EVoiceCaptureState::Ok) {
				UE_LOG(LogVoiceChatPro, Verbose, TEXT("Voice capture state is not OK: %d"), (int32)CaptureState);
			}
			...
			if (CaptureState == EVoiceCaptureState::Ok && VoiceCaptureBytesAvailable > 0)
			{
				...
				if (frameSentToOpus > 0) {
					...
					PCVoiceCaptureBufferAppended.RemoveAt(0, uint8sizeBufferPCMMic, EAllowShrinking::No);
				}
			}
		}
#endif
...
void UMicrophoneSpeakComponent::SetVoiceSoundClass(USoundClass* soundClass) {
	if (VoiceAudioComponent != NULL) {
		VoiceAudioComponent->SoundClassOverride = soundClass;
	}

	if (VoiceSoundWaveProcedural != NULL) {
		VoiceSoundWaveProcedural->SoundClassObject = soundClass;
	}

	UE_LOG(LogVoiceChatPro, Display, TEXT("UMicrophoneSpeakComponent::SetVoiceSoundClass %s"), soundClass ? *soundClass->GetName() : TEXT("None"));
}
...
bool UMicrophoneSpeakComponent::startSpeaking(bool _shouldHearMyOwnVoice, bool _isGlobal, TArray<int32> _radioChannel, bool _useRange, float _maxRange) {
	...
	UE_LOG(LogVoiceChatPro, Warning, TEXT("[VoiceChatPlugin] UMicrophoneSpeakComponent::startSpeaking CALLED. ShouldHearOwnVoice: %d, IsGlobal: %d"), _shouldHearMyOwnVoice, _isGlobal);
...
#if PLATFORM_WINDOWS || PLATFORM_MAC || PLATFORM_LINUX	
	UE_LOG(LogVoiceChatPro, Display, TEXT("UMicrophoneSpeakComponent::startSpeaking desktop capture start"));
	if (PCVoiceCapture != NULL && PCVoiceCapture.IsValid()) {
		PCVoiceCapture->Start();
		if (!PCVoiceCapture->IsCapturing()) {
			UE_LOG(LogVoiceChatPro, Warning, TEXT("UMicrophoneSpeakComponent::startSpeaking voice capture did not start"));
			return false;
		}
		isSpeakingLocalSetting = true;
		return true;
	}	
	UE_LOG(LogVoiceChatPro, Warning, TEXT("UMicrophoneSpeakComponent::startSpeaking PCVoiceCapture is invalid"));
	return false;
#else
	UE_LOG(LogVoiceChatPro, Warning, TEXT("UMicrophoneSpeakComponent::startSpeaking no desktop voice capture implementation for this platform"));
	return false;
#endif
}
...
void UMicrophoneSpeakComponent::receiveDataMicrophone(TArray<int16> data, int32 readsize) {
	...
	if (frameSentToOpus > 0) {
		...
		// remove transmited data
		MobileVoiceCaptureBufferAppended.RemoveAt(0, uint8sizeBufferPCMMic, EAllowShrinking::No);
	}
